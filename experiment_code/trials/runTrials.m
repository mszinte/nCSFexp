...
for n_trial = 1:const.trialsNum
    ...
    if var1 == 7
        ...
        if time2probe_cond(n_trial)
            screen_filename = sprintf('%s/%sprobe_sfStim%i_contStim%i_kappaStim%i_noiseRand%i.mat', ...
                const.stim_folder, var1, var2, expDes.stim_stair_val, rand1);
        else
            screen_filename = sprintf('%s/%snoprobe_sfStim%i_contStim%i_noiseRand%i.mat', ...

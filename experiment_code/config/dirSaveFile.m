...
if ~isfolder(sprintf('data/%s/%s/%s/%s/', const.task, const.sjct, const.session, ...
        const.modality))
    mkdir(sprintf('data/%s/%s/%s/%s/', const.task, const.sjct, const.session, ...
        const.modality))
end

% Define directory
const.dat_output_dir = sprintf('data/%s/%s/%s/%s',...
                               const.task, const.sjct, const.session, const.modality);
const.dat_output_dir_sess1 = sprintf('data/%s/%s/ses-01/%s',...
                                     const.task,const.sjct, const.modality);
